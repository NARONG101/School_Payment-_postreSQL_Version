...
<head>
    ...
    <title>@yield('title', 'Student Payment System')</title>
    ...
    <style>
        ...
        *:focus-visible {
        ...
        .alert-level-overdue td { background: #fff5f5 !important; }
        .alert-level-critical td { background: #fffbf0 !important; }
        .deadline-pill {
            font-size: 11px; font-weight: 700;
        ...
        .deadline-warning  { background: #fef3c7; color: #b45309; }
        ...
        @media (min-width: 1025px) {
            .sidebar { transform: translateX(0); transition: width 0.3s ease; }
            .sidebar.closed { width: 80px; }
            .sidebar.closed .brand-text,
            .sidebar.closed .nav-link span,
            .sidebar.closed .user-info .user-details,
            ...
            .sidebar.closed .brand-header {
                justify-content: center;
            }
            .main-content { margin-left: var(--sidebar-w); transition: margin-left 0.3s ease; }
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-brand">
        <div class="brand-header">
            ...
            <button class="sidebar-toggle-btn" id="sidebarToggleBtn" aria-label="Toggle sidebar" aria-expanded="true">
                <i class="fas fa-bars sidebar-toggle-icon"></i>
            </button>
        </div>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('payments.alerts') }}" class="nav-link {{ request()->routeIs('payments.alerts') ? 'active' : '' }}">
            <i class="fas fa-bell"></i> <span>Deadline Alerts</span>
            @php
                // Student-based alert count for sidebar
                $now = now();
                ...
                foreach ($students as $student) {
                    ...
                    if ($lastPayment && $lastPayment->payment_date) {
                        $nextPaymentDate = \Illuminate\Support\Carbon::parse($lastPayment->payment_date)->addMonth();
            ...
            @endphp
            @if($alertCount > 0)
                <span class="badge badge-danger ms-auto user-details" style="margin-left:auto">{{ $alertCount }}</span>
            @endif
        </a>
    ...
    <div class="sidebar-footer">
        <div class="user-info">
            ...
            <form action="{{ route('logout') }}" method="POST" style="margin-left:auto" class="user-details">
                @csrf
                <button type="submit" class="btn btn-icon btn-outline" title="Logout" style="color:var(--gray-400);border-color:rgba(255,255,255,0.1);background:transparent">
...
function toggleSidebar() {
    ...
    if (isDesktop) {
        ...
        sidebarToggleBtn.setAttribute('aria-expanded', !isClosed);
    } else {
...
function openDeleteModal(actionUrl, title = 'Delete Item', body = 'Are you sure you want to delete this item? This action cannot be undone.') {
    deleteModalTitle.textContent = title;
    deleteModalBody.textContent = body;
...
document.addEventListener('click', (e) => {
    ...
    if (deleteBtn) {
        e.preventDefault();
        const actionUrl = deleteBtn.dataset.action;
        const title = deleteBtn.dataset.title || 'Delete Item';
        const body = deleteBtn.dataset.body || 'Are you sure you want to delete this item? This action cannot be undone.';
        openDeleteModal(actionUrl, title, body);
    }
});
</script>
</body>
